...
class BoundaryConditions(PDE):
    ...
    >>> bc.pprint()
    neumann_phi: phi__x
    flux_cp: -cp*phi__x - cp__x - 0.1
    flux_cn: cn*phi__x - cn__x
    """
    ...
    def __init__(self, delta=1.0):
        ...
        # left boundary (x=0)
        self.equations["neumann_phi"] = (
            phi.diff(x, 1)
        )
        self.equations["flux_cp"] = (
            -cp.diff(x, 1) - cp * phi.diff(x, 1) - delta
        )
        self.equations["flux_cn"] = (
            -cn.diff(x, 1) + cn * phi.diff(x, 1)
        )


@physicsnemo.sym.main(config_path="conf", config_name="config_decoupled")
