...
class PNPValidatorPlotter(ValidatorPlotter):
    ...
    times: Tuple[float], optional
        Times plot spatial profiles at, default is (0.0, 1.0, 6.0, 36.0, 100.0, 3600.0)
    parameters: Type[Parameters], optional
        Instance of physical parameters class for PNP system, default is Parameters()
    """
    ...
    def __call__(self, invar, true_outvar, pred_outvar):
        ...
        colors = cmap(np.linspace(0, 1, len(self.times)))

        # group plots by time
        for i, t in enumerate(self.times):
            target_y = t / self.parameters.t_c
